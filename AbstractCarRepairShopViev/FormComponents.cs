...
namespace AbstractCarRepairShopViev
{
    public partial class FormComponents : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                Program.ConfigGrid(logic.Read(null), dataComponentsGridView);
            }
            catch (Exception ex)
            {
        ...
        {
            if (dataComponentsGridView.SelectedRows.Count == 1)
            {
                var form = Container.Resolve<FormComponent>();
                form.Id = Convert.ToInt32(dataComponentsGridView.SelectedRows[0].Cells[0].Value);
                if (form.ShowDialog() == DialogResult.OK)
                {
                    LoadData();
                }
        ...
        private void ButtonDel_Click(object sender, EventArgs e)
        {
            if (dataComponentsGridView.SelectedRows.Count == 1)
            {
                ...
                {
                    int id = Convert.ToInt32(dataComponentsGridView.SelectedRows[0].Cells[0].Value);
                    try
                    {
